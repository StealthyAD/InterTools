--[[
    ...
    ----========================================----
        ...
        SelfParts:toggle("Partial Invisible", {}, "Turn partially invisible mode (Players will not able to see you), but you will see only yourself, includes vehicles.", function(toggle)
            local mode = toggle and "on" or "off"
   ...
   ----========================================----
    ...
    ----========================================----
        ...
        local gearShiftSpeeds = {50, 96, 134, 172, 196, 220}
        local currentGear = 1
        local lastGearShiftTime = 0
        local isSpeedReduced = false
        local wasSpeedReduced = false
        
        VehicleParts:toggle_loop("Manual Gearbox", {"intermanualgearbox"}, "Press X to change gearbox (1 to 6).", function()
            local player = players.user_ped()
            local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(player, true)
            if PED.IS_PED_IN_VEHICLE(player, playerVehicle, false) then
                local currentSpeed = ENTITY.GET_ENTITY_SPEED(playerVehicle) * 3.6
                if PAD.IS_CONTROL_PRESSED(323, 323) then
                    local currentTime = os.time()
                    local timeSinceLastGearShift = currentTime - lastGearShiftTime
                    if timeSinceLastGearShift >= 1 then
                        lastGearShiftTime = currentTime
                        if currentGear < #gearShiftSpeeds then
                            currentGear = currentGear + 1
                            InterNotify("Gear change: " .. currentGear)
                            local maxSpeed = gearShiftSpeeds[currentGear]
                            local newSpeed = maxSpeed / 3.6
                            VEHICLE.SET_VEHICLE_MAX_SPEED(playerVehicle, newSpeed)
                        else
                            InterNotify("You are already at top speed.")
                        end
                    end
                end
                if currentSpeed < #gearShiftSpeeds then
                    if currentGear > 1 then
                        local gearSpeed = gearShiftSpeeds[currentGear]
                        if currentSpeed < gearSpeed then
                            currentGear = currentGear - 1
                            InterNotify("Gear change: " .. currentGear)
                ...
                local maxSpeed = gearShiftSpeeds[currentGear]
                if currentSpeed > maxSpeed and currentSpeed <= 220 then
                    local newSpeed = maxSpeed / 3.6
                    VEHICLE.SET_VEHICLE_MAX_SPEED(playerVehicle, newSpeed)
                elseif currentSpeed > 220 then
                    VEHICLE.SET_VEHICLE_MAX_SPEED(playerVehicle, 10000.0)
                end
                if currentGear > 1 then
                    if currentSpeed < gearShiftSpeeds[currentGear - 1] and not isSpeedReduced then
                        isSpeedReduced = true
                    elseif currentSpeed >= gearShiftSpeeds[currentGear - 1] and isSpeedReduced then
                        isSpeedReduced = false
                    end
                end
                if currentGear > 1 and isSpeedReduced and not wasSpeedReduced then
                    if currentSpeed < gearShiftSpeeds[currentGear - 1] then
                        currentGear = currentGear - 1
                        InterNotify("Gear change: " .. currentGear)
                ...
                wasSpeedReduced = isSpeedReduced
                if currentSpeed == gearShiftSpeeds[1] and currentGear == 1 then
                    InterNotify("Gearbox changed to 1")
                end
            end
        end, function()
            local player = players.user_ped()
   ...
   ----========================================----
    ...
    ----========================================----    
        ...
        DetectionRadar:toggle_loop("Car Stealth Radar", {}, "Only works for ground vehicles.", function()
            ...
            local altitude = 5
            local altitudeBike = 75
            local ped = PLAYER.PLAYER_PED_ID()
            local player_veh = PED.GET_VEHICLE_PED_IS_USING(ped)
            local isOnBike = PED.IS_PED_ON_ANY_BIKE(ped)
            local isOnVehicle = PED.IS_PED_IN_ANY_VEHICLE(ped, false)
            local isNotOnAirVehicle = not PED.IS_PED_IN_ANY_HELI(ped) and not PED.IS_PED_IN_ANY_PLANE(ped)
            local heightAboveGround = ENTITY.GET_ENTITY_HEIGHT_ABOVE_GROUND(player_veh)
        
            if (isOnBike and heightAboveGround < altitudeBike) or 
               (isOnVehicle and isNotOnAirVehicle and heightAboveGround < altitude) then
    ...
    ----========================================----
        ...
        local AerialRoots = SessionRoots:list("Aerial Fleets & Ground Defense")
        ...
        local WantedRoots = SessionRoots:list("Wanted Parts")
    ...
    ----========================================----
        ...
        CustomVehicleTF:text_input("Delay Countdown", {"intercountdowntf"}, "Countdown for messages.\nMeasured in seconds.", function(typeText)
            if typeText ~= "" then
                local delay = tonumber(typeText)
        ...
        CustomVehicleTF:action("Send Air Force (Task Force) - Custom Surface", {}, "Sending America to war and intervene more custom planes (Real Undetectable).\nWARNING: The action is irreversible in the session bcz godmode permanent.\n\nSome peds can fall and attach you.", function()
            if menu.get_value(CustomVehicleAdvanced) ~= true then InterNotify("I'm sorry, enable \"Custom Vehicle\" to use more advantages.") return end
            local player = PLAYER.PLAYER_PED_ID()
            local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(player, true)
            ...
            if vehicleClass ~= 19 then InterNotify("To operate the action, you need to be in a military vehicle.") return end
            local playerList = players.list(false, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)
            ...
            if vehicleClass ~= 15 and vehicleClass ~= 16 then
                InterNotify("Invalid vehicle model: " .. textInput .. ". Only aerial vehicles planes or helicopters are allowed.")
                return
            end
            for _, pid in pairs(playerList) do
        ...
        CustomVehicleTF:action("Send Air Force (Task Force) - Custom Aerial", {}, "Sending America to war and intervene more custom planes (Real Undetectable).\nWARNING: The action is irreversible in the session bcz godmode permanent.\n\nSome peds can fall and attach you.", function()
            ...
            local vehicleClass = VEHICLE.GET_VEHICLE_CLASS_FROM_NAME(modelHash)
            if vehicleClass ~= 15 and vehicleClass ~= 16 then
                InterNotify("Invalid vehicle model: " .. textInput .. ". Only aerial vehicles planes are allowed.")
                return
            end
            for _, pid in pairs(playerList) do
                if AvailableSession() and not players.is_in_interior(pid) then
    ...
    ----========================================----
        ...
        TaskForce:text_input("Model Vehicle", {"intermodelveh"}, "Choose specific model existing on GTAV or existing DLC Customs Aerial Vehicles. Recommended to use combat fighters planes", function(txtModel)
            if txtModel ~= "" then
        ...
        TaskForce:action("Target Player", {"intertarget"}, "We need more communication and more precise for informations.\nTarget player is the priority objective for your choice if the player is in the session.", function()
            ...
            if isRandomToggle then
                ...
                if not AvailableSession() then
                    InterNotify("Unable to target player due to session availability.")
                    return
                end
                ...
                if not players.is_in_interior(playerId) then
                    ...
                    InterNotify("I'm sorry, you cannot target "..playerName.." while staying on the base. But I have an idea to force. Let's US Army do something.")
                    for i = 1, 5 do
                ...
                for _, pid in ipairs(playerList) do
                    ...
                    if playerName == textInput then
                        ...
                        if not players.is_in_interior(pid) then
                            ...
                            InterNotify("I'm sorry, you cannot target "..textInput.." while staying on the base. But I have an idea to force. Let's US Army do something.")
                            for i = 1, 5 do
    ...
    ----========================================----
        ...
        PresetSpawningTF:text_input("Send Message", {"intermsgpf"}, "America has sent a friend request.", function(typeText)
            if typeText ~= "" then
                msgPresets = typeText
            else
        ...
        PresetSpawningTF:text_input("Delay Countdown", {"intercountdownpf"}, "Countdown for notification.\nMeasured in seconds.", function(typeText)
            if typeText ~= "" then
                local delay = tonumber(typeText)
                if delay and delay > 1 then
        ...
        PresetSpawningTF:text_input("Delay Time", {"intertimepf"}, "Do not abuse for spawning vehicle, do not go to lower for preventing for crash, mass entities.\n\nMeasured in seconds.", function(typeText)
            if typeText ~= "" then
                local delay = tonumber(typeText)
                if delay and delay > 0 then
        ...
        for _, spawner in ipairs(tempSpawners) do
            ...
            PresetSpawningTF:action("Spawn " .. spawnerName, {"intertask"..spawnerModel}, "", function()
                local player = PLAYER.PLAYER_PED_ID()
                local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(player, true)
    ...
    ----========================================----
        ...
        GroundParts:divider("Ground Defense (Cops/US Army)")
        ...
        GroundParts:action("Send Ground Army (Cops)", {"intercops"}, "Sending Cops to war and intervene more ground vehicles.\nWARNING: The action is irreversible in the session if toggle godmode on.\nNOTE: Toggle Exclude features", function()
            local playerList = players.list(EToggleSelf, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)
            local vehicleOptions = {"police5", "polgauntlet", "police4"} -- Ajoutez d'autres options ici si nécessaire
            for _, pid in pairs(playerList) do
                if AvailableSession() then
                    local randomVehicle = vehicleOptions[math.random(#vehicleOptions)] -- Sélectionne aléatoirement un véhicule
                    groundAttack(pid, randomVehicle, true)
                    InterWait(750)
                end
            end
        end)

        GroundParts:action("Send Ground Army (US Army)", {"interusarmy"}, "Sending America to war and intervene more ground vehicles.\nWARNING: The action is irreversible in the session if toggle godmode on.\nNOTE: Toggle Exclude features", function()
            local playerList = players.list(EToggleSelf, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)
            local vehicleOptions = {"rhino", "apc", "crusader"} -- Ajoutez d'autres options ici si nécessaire
            for _, pid in pairs(playerList) do
                if AvailableSession() then
                    local randomVehicle = vehicleOptions[math.random(#vehicleOptions)] -- Sélectionne aléatoirement un véhicule
                    groundAttack(pid, randomVehicle, true)
        ...
        ExplodeRoot:action_slider("Orbital Cannon Type", {}, "Say hi to the sky", {"Non-Personal", "Owned", "Randomize"}, function(orbSelect)
            for _, pid in pairs(players.list(false, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)) do
                if not players.is_in_interior(pid) and not players.is_godmode(pid) then
                    local pos, ped = prepareExplosion(pid)
                    ...
                    elseif orbSelect == 2 then
                        ...
                        if AvailableSession() and players.get_name(pid) ~= "UndiscoveredPlayer" then
                            local playerList = players.list(false, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)
                            local randomIndex = math.random(#playerList)
                            local randomPlayerId = playerList[randomIndex]
                            local randomPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(randomPlayerId)
                            FIRE.ADD_OWNED_EXPLOSION(randomPed, pos.x, pos.y, pos.z, 59, 1, true, false, 9.9, false)
                        end
                    end
                    triggerExplosion(pos, ped)
                end
            end
        end)
    ...
    ----========================================----
    ---              Wanted Parts
    ---    The part of session wanted online
    ----========================================----

        WantedLevels = WantedRoots:slider("Wanted Level Choice", {"intwantedc"}, "Choose Wanted amount.", 0, 5, 0, 1, function()end)

        WantedRoots:toggle_loop("Wanted Level", {"intwc"}, "Enable Wanted Level for all Players", function()
            local playerList = players.list(false, false, EToggleStrangers, EToggleCrew, EToggleOrg)
            for _, pid in pairs(playerList) do
                if AvailableSession() and players.get_name(pid) ~= "UndiscoveredPlayer" then
                    players.set_wanted_level(pid, menu.get_value(WantedLevels))
                end
            end
        end)

        WantedRoots:toggle_loop("Most Wanted", {"intwantedall"}, "Enable Most Wanted Level for all Players", function()
            local playerList = players.list(false, false, EToggleStrangers, EToggleCrew, EToggleOrg)
            for _, pid in pairs(playerList) do
                if AvailableSession() and players.get_name(pid) ~= "UndiscoveredPlayer" then
                    players.set_wanted_level(pid, 5)
                end
            end
        end)

        WantedRoots:toggle_loop("Never Wanted", {"intneverwantedall"}, "Disable Wanted Level for all Players", function()
            local playerList = players.list(false, false, EToggleStrangers, EToggleCrew, EToggleOrg)
            for _, pid in pairs(playerList) do
                if AvailableSession() and players.get_name(pid) ~= "UndiscoveredPlayer" then
                    players.set_wanted_level(pid, 0)
                end
            end
        end)
    ...
    ----========================================----
        ...
        SessionRoots:action_slider("Random Action Player", {}, "Different types of player actions:\n- Teleport Player\n- Kick Player\n- Crash Player\n- Bounty Player (+ Custom Bounty)\n- Wanted Level", {"Teleport", "Kick", "Crash", "Bounty", "Custom Bounty", "Wanted"}, function(randselect)
            ...
            elseif randselect == 5 then
                ...
                local playerList = players.list(false, EToggleFriend, EToggleStrangers, EToggleCrew, EToggleOrg)
                if #playerList > 0 then
                    local wantedRand = math.random(1, 5)
                    local randomIndex = math.random(#playerList)
                    local playerId = playerList[randomIndex]  -- Utilisez l'ID du joueur, pas le nom
                    local playerName = players.get_name(playerId)
                    InterNotify("Player name randomly mostly wanted: "..playerName.."\nWanted Randomized: "..wantedRand)
                    players.set_wanted_level(playerId, wantedRand)  -- Utilisez l'ID ici
                else
                    InterNotify("No players are currently in the session.")
                end                
            end
    ...
    ----========================================----
        ...
        ----========================================----
            ...
            "\n- Kosatka\n- Cargo Plane\n- Boeing 911\n- B-1B Lancer\n- Rhino Tank", TablesEntities, function(spamType)
                local vehicleHashes = {
                    util.joaat("kosatka"),
                    util.joaat("cargoplane"),
                    util.joaat("jet"),
                    util.joaat("alkonost"),
                    util.joaat("rhino")
                }
            
                local hash = vehicleHashes[spamType]
                if not STREAMING.HAS_MODEL_LOADED(hash) then
                    request_model_load(hash)
                end
                ...
                local vehicle = entities.create_vehicle(hash, c, ENTITY.GET_ENTITY_HEADING(ped))
                for i = 0, 49 do
                    local num = VEHICLE.GET_NUM_VEHICLE_MODS(vehicle, i)
                    VEHICLE.SET_VEHICLE_MOD(vehicle, i, num - 1, true)
                end
            ...
            TrollingOptions:action_slider("Send Plane", {}, "Call the Plane to send "..InterName.." to die.\n\nBOEING IS THE FASTEST PLANE EVER THAN SHITTY PLANES.", {"Boeing 747","F-16 Falcon","Antonov AN-225"}, function(select)
                ...
                elseif select == 2 then
                    ...
                    local function GiveSPlane(pid)
                        ...
                        for i = 1, 20 do
                            VEHICLE.SET_VEHICLE_FORWARD_SPEED(lazersuicide, 1000.0)
                            VEHICLE.SET_VEHICLE_MAX_SPEED(lazersuicide, 1000.0)
                        end
